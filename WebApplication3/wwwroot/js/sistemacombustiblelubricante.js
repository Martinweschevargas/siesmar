var tblSistemaCombustibleLubricantes;
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                ...
                                success: function (mensaje) {
                                    ...
                                    $('#listar').show();
                                    $('#nuevo').hide();
                                    $('#tblSistemaCombustibleLubricantes').DataTable().ajax.reload();
                                },
                                complete: function () {
                                    $('#loader-6').hide();
                                }
        ...
        "columns": [
            ...
            { "data": "descSistemaCombustibleLubricante" },
            {
                "render": function (data, type, row) {
                    return '<a class="txt" onclick=edit(' + row.sistemaCombustibleLubricanteId + ') title="Actualizar"><i class="fa fa-check-square-o" aria-hidden="true" style="color:black; padding-right:5px"></i>Editar</a>';
                }
            },
            {
                "render": function (data, type, row) {
                    return '<a class="txt btnconfirmation" onclick=eliminar(' + row.sistemaCombustibleLubricanteId + ') title="Eliminar"><i class="fa fa-minus-square-o red" aria-hidden="true" style="color:red; padding-right:5px"></i>Eliminar</a>';
                }
            }
        ],
        language: {
...
function edit(SistemaCombustibleLubricanteId) {
    $('#listar').hide();
    $('#editar').show();
...
function eliminar(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                type: "POST",
                url: '/SistemaCombustibleLubricante/EliminarSistemaCombustibleLubricante',
                data: {
                    'SistemaCombustibleLubricanteId': id
                },
                beforeSend: function () {
                    $('#loader-6').show();
                },
...
function nuevaSistemaCombustibleLubricante() {
    $('#listar').hide();
    $('#nuevo').show();
}
