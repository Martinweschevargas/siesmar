var tblComoperpacBajaTiempoPaz;
var distritoUbigeo;
var provinciaUbigeo;
...
$(document).ready(function () {
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                ...
                                success: function (mensaje) {
                                    ...
                                    $('#listar').show();
                                    $('#nuevo').hide();
                                    $('#tblComoperpacBajaTiempoPaz').DataTable().ajax.reload();
                                },
                                complete: function () {
                                    $('#loader-6').hide();
                                }
                ...
                if (!form.checkValidity()) {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                type: "POST",
                                url: '/ComoperpacBajaTiempoPaz/Actualizar',
                                data: {
                                    'Id': $('#txtCodigo').val(),
                                    ...
                                    'MotivoBajaTiempoPaz': $('#txtMotivoBajaTiempoPaze').val(),
                                },
                                beforeSend: function () {
                                    $('#loader-6').show();
                                },
    ...
    $('#tblComoperpacBajaTiempoPaz').DataTable({
        ajax: {
            "url": '/ComoperpacBajaTiempoPaz/CargaTabla',
            "type": "GET",
            "datatype": "json"
        },
        "columns": [
            { "data": "bajaTiempoPazId" },
            { "data": "descZonaNaval" },
            ...
            { "data": "descTipoBaja" },
            { "data": "motivoBajaTiempoPaz" },

            {
                "render": function (data, type, row) {
                    return '<a class="txt" onclick=edit(' + row.bajaTiempoPazId + ') title="Actualizar"><i class="fa fa-check-square-o" aria-hidden="true" style="color:black; padding-right:5px"></i>Editar</a>';
                }
            },
            {
                "render": function (data, type, row) {
                    return '<a class="txt btnconfirmation" onclick=eliminar(' + row.bajaTiempoPazId + ') title="Eliminar"><i class="fa fa-minus-square-o red" aria-hidden="true" style="color:red; padding-right:5px"></i>Eliminar</a>';
                }
            }
        ],
        language: {
        ...
        buttons: [
            //csv,
            {
                extend: 'csvHtml5',
                text: 'Exportar CSV',
                filename: 'Comoperpac - Bajas en Tiempo de Paz',
                title: '',
                exportOptions: {
            ...
            //'excel',
            {
                ...
                text: 'Exportar Excel',
                filename: 'Comoperpac - Bajas en Tiempo de Paz',
                title: 'Comoperpac - Bajas en Tiempo de Paz',
                exportOptions: {
                    columns: [0, 1, 2, 3, 4, 5, 6, 7]
                },
...
function edit(Id) {
    $('#listar').hide();
    $('#editar').show();
    $.getJSON('/ComoperpacBajaTiempoPaz/Mostrar?Id=' + Id, [], function (BajaTiempoPazDTO) {
        $('#txtCodigo').val(BajaTiempoPazDTO.bajaTiempoPazId);
        $('#cbZonaNavale').val(BajaTiempoPazDTO.zonaNavalId);
        $('#cbDepartamentoe').val(BajaTiempoPazDTO.departamentoUbigeoId);
        $('#cbProvinciae').val(BajaTiempoPazDTO.provinciaUbigeoId);
        $('#cbDistritoe').val(BajaTiempoPazDTO.distritoUbigeoId);
        $('#cbGradoPersonale').val(BajaTiempoPazDTO.gradoPersonalId);
        $('#cbTipoBajae').val(BajaTiempoPazDTO.tipoBajaId);
        $('#txtMotivoBajaTiempoPaze').val(BajaTiempoPazDTO.motivoBajaTiempoPaz);
    });
}
...
function eliminar(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                type: "POST",
                url: '/ComoperpacBajaTiempoPaz/Eliminar',
                data: {
                    'Id': id
                },
                beforeSend: function () {
...
function nuevaComoperpacBajaTiempoPaz() {
    $('#listar').hide();
    $('#nuevo').show();
}
...
function mostrarDatos() {
    ...
    fetch("ComoperpacBajaTiempoPaz/MostrarDatos", {
        method: "POST",
        body: formData
    })
            ...
            dataJson.forEach((item) => {
                $("#tbData tbody").append(
                    $("<tr>").append(
                        ...
                        $("<td>").text(item.motivoBajaTiempoPaz),
                    )
                )
            })
        })
...
function enviarDatos() {
    ...
    formData.append("ArchivoExcel", input.files[0])
    fetch("ComoperpacBajaTiempoPaz/EnviarDatos", {
        method: "POST",
        body: formData
    })
        .then((response) => { return response.json() })
...
function cargaDatos() {
    ...
    $.getJSON('/ComoperpacBajaTiempoPaz/cargaCombs', [], function (Json) {
