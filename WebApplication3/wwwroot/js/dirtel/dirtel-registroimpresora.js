var tblDirtelRegistroImpresora;
...
$(document).ready(function () {
                ...
                if (!form.checkValidity()) {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                type: "POST",
                                url: '/DirtelRegistroImpresora/Insertar',
                                data: {
                                    'CodigoIBPImpresora': $('#txtCodigoIBP').val(),
                                    'ModeloBienServicioSubcampoId': $('#cbSubcampo').val(),
                                    'ModeloBienServicioDenominacionId': $('#cbDenominacion').val(),
                                    'ColorImpresion': $('#txtColor').val(),
                                    'MarcaId': $('#cbMarca').val(),
                                ...
                                success: function (mensaje) {
                                    ...
                                    $('#listar').show();
                                    $('#nuevo').hide();
                                    $('#tblDirtelRegistroImpresora').DataTable().ajax.reload();
                                },
                                complete: function () {
                                    $('#loader-6').hide();
                                }
                ...
                if (!form.checkValidity()) {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                type: "POST",
                                url: '/DirtelRegistroImpresora/Actualizar',
                                data: {
                                    'RegistroImpresoraId': $('#txtCodigo').val(),
                                    'CodigoIBPImpresora': $('#txtCodigoIBPe').val(),
                                    'ModeloBienServicioSubcampoId': $('#cbSubcampoe').val(),
                                    'ModeloBienServicioDenominacionId': $('#cbDenominacione').val(),
                                    'ColorImpresion': $('#txtColore').val(),
    ...
    $('#tblDirtelRegistroImpresora').DataTable({
        ajax: {
            "url": '/DirtelRegistroImpresora/CargaTabla',
            "type": "GET",
            "datatype": "json"
        },
        "columns": [
            { "data": "registroImpresoraId" },
            { "data": "codigoIBPImpresora" },
            ...
            { "data": "departamentoUbigeoId" },   
            {
                "render": function (data, type, row) {
                    return '<a class="txt" onclick=edit(' + row.registroImpresoraId + ') title="Actualizar"><i class="fa fa-check-square-o" aria-hidden="true" style="color:black; padding-right:5px"></i>Editar</a>';
                }
            },
            {
                "render": function (data, type, row) {
                    return '<a class="txt btnconfirmation" onclick=eliminar(' + row.registroImpresoraId + ') title="Eliminar"><i class="fa fa-minus-square-o red" aria-hidden="true" style="color:red; padding-right:5px"></i>Eliminar</a>';
                }
            }
        ],
        language: {
...
function edit(Id) {
    $('#listar').hide();
    $('#editar').show();
    $.getJSON('/DirtelRegistroImpresora/Mostrar?Id=' + Id, [], function (RegistroImpresoraDTO) {
        $('#txtCodigo').val(RegistroImpresoraDTO.registroImpresoraId);
        $('#txtCodigoIBPe').val(RegistroImpresoraDTO.codigoIBPImpresora);
        $('#cbSubcampoe').val(RegistroImpresoraDTO.modeloBienServicioSubcampoId);
        $('#cbDenominacione').val(RegistroImpresoraDTO.modeloBienServicioDenominacionId);
        $('#txtColore').val(RegistroImpresoraDTO.colorImpresion);
        $('#cbMarcae').val(RegistroImpresoraDTO.marcaId);
        $('#txtAnioAdquisicione').val(RegistroImpresoraDTO.anioAdquisicionServidor);
        $('#txtEstadoOperatividade').val(RegistroImpresoraDTO.estadoOperatividadServidor);
        $('#cbDependenciae').val(RegistroImpresoraDTO.dependenciaId);
        $('#cbDepartamentoe').val(RegistroImpresoraDTO.departamentoUbigeoId); 
    });
}
...
function eliminar(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                type: "POST",
                url: '/DirtelRegistroImpresora/Eliminar',
                data: {
                    'Id': id
                },
                beforeSend: function () {
...
function nuevaDirtelRegistroImpresora() {
    $('#listar').hide();
    $('#nuevo').show();
}
...
function cargaDatos() {
    $.getJSON('/DirtelRegistroImpresora/cargaCombs', [], function (Json) {
