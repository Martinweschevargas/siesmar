var tblDirtelRegistroEquipoPeriferico;
...
$(document).ready(function () {
                ...
                if (!form.checkValidity()) {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                type: "POST",
                                url: '/DirtelRegistroEquipoPeriferico/Insertar',
                                data: {
                                    'CodigoIBPPeriferico': $('#txtCodigoIBP').val(),
                                ...
                                success: function (mensaje) {
                                    ...
                                    $('#listar').show();
                                    $('#nuevo').hide();
                                    $('#tblDirtelRegistroEquipoPeriferico').DataTable().ajax.reload();
                                },
                                complete: function () {
                                    $('#loader-6').hide();
                                }
                ...
                if (!form.checkValidity()) {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                type: "POST",
                                url: '/DirtelRegistroEquipoPeriferico/Actualizar',
                                data: {
                                    'RegistroEquipoPerifericoId': $('#txtCodigo').val(),
                                    'CodigoIBPPeriferico': $('#txtCodigoIBPe').val(),
    ...
    $('#tblDirtelRegistroEquipoPeriferico').DataTable({
        ajax: {
            "url": '/DirtelRegistroEquipoPeriferico/CargaTabla',
            "type": "GET",
            "datatype": "json"
        },
        "columns": [
            { "data": "registroEquipoPerifericoId" },
            { "data": "codigoIBPPeriferico" },
            ...
            { "data": "dependenciaId" },  
            { "data": "departamentoUbigeoId" },   
            {
                "render": function (data, type, row) {
                    return '<a class="txt" onclick=edit(' + row.registroEquipoPerifericoId + ') title="Actualizar"><i class="fa fa-check-square-o" aria-hidden="true" style="color:black; padding-right:5px"></i>Editar</a>';
                }
            },
            {
                "render": function (data, type, row) {
                    return '<a class="txt btnconfirmation" onclick=eliminar(' + row.registroEquipoPerifericoId + ') title="Eliminar"><i class="fa fa-minus-square-o red" aria-hidden="true" style="color:red; padding-right:5px"></i>Eliminar</a>';
                }
            }
        ],
        language: {
        ...
        buttons: [
            //csv,
            {
                extend: 'csvHtml5',
                text: 'Exportar CSV',
                filename: 'Dirtel - Equipos Perifericos',
                title: '',
                exportOptions: {
            ...
            //'excel',
            {
                ...
                text: 'Exportar Excel',
                filename: 'Dirtel - Equipos Perifericos',
                title: 'Dirtel - Equipos Perifericos',
                exportOptions: {
                    columns: [0, 1, 2, 3, 4, 5, 6, 7, 8]
                },
...
function edit(Id) {
    $('#listar').hide();
    $('#editar').show();
    $.getJSON('/DirtelRegistroEquipoPeriferico/Mostrar?Id=' + Id, [], function (RegistroEquipoPerifericoDTO) {
        $('#txtCodigo').val(RegistroEquipoPerifericoDTO.registroEquipoPerifericoId);
        $('#txtCodigoIBPe').val(RegistroEquipoPerifericoDTO.codigoIBPPeriferico);
        $('#cbSubcampoe').val(RegistroEquipoPerifericoDTO.modeloBienServicioSubcampoId);
        $('#cbDenominacione').val(RegistroEquipoPerifericoDTO.modeloBienServicioDenominacionId);
        $('#cbMarcae').val(RegistroEquipoPerifericoDTO.marcaId);
        $('#txtAnioAdquisicione').val(RegistroEquipoPerifericoDTO.anioAdquisicionServidor);
        $('#txtEstadoOperatividade').val(RegistroEquipoPerifericoDTO.estadoOperatividadServidor);
        $('#cbDependenciae').val(RegistroEquipoPerifericoDTO.dependenciaId);
        $('#cbDepartamentoe').val(RegistroEquipoPerifericoDTO.departamentoUbigeoId);   
    });
}
...
function eliminar(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                type: "POST",
                url: '/DirtelRegistroEquipoPeriferico/Eliminar',
                data: {
                    'Id': id
                },
                beforeSend: function () {
...
function nuevaDirtelRegistroEquipoPeriferico() {
    $('#listar').hide();
    $('#nuevo').show();
}
...
function cargaDatos() {
    $.getJSON('/DirtelRegistroEquipoPeriferico/cargaCombs', [], function (Json) {
