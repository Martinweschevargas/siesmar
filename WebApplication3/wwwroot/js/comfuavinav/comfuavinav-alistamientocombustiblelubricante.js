...
$(document).ready(function () {
                ...
                if (!form.checkValidity()) {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                type: "POST",
                                url: '/ComfuavinavAlistamientoCombustibleLubricante/Insertar',
                                data: {
                                ...
                                success: function (mensaje) {
                                    ...
                                    $('#listar').show();
                                    $('#nuevo').hide();
                                    $('#tblComfuavinavAlistamientoCombustibleLubricante').DataTable().ajax.reload();
                                },
                                complete: function () {
                                    $('#loader-6').hide();
                                }
                ...
                if (!form.checkValidity()) {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                type: "POST",
                                url: '/ComfuavinavAlistamientoCombustibleLubricante/Actualizar',
                                data: {
                                    'Id': $('#txtCodigo').val(),
    ...
    tblComfuavinavAlistamientoCombustibleLubricante = $('#tblComfuavinavAlistamientoCombustibleLubricante').DataTable({
        ...
        "columns": [
            ...
            {
                "render": function (data, type, row) {
                    return '<a class="txt btnconfirmation" onclick=eliminar(' + row.alistamientoCombustibleLubricanteComfuavinavId + ') title="Eliminar"><i class="fa fa-minus-square-o red" aria-hidden="true" style="color:red; padding-right:5px"></i>Eliminar</a>';
                }
            }
        ],
        language: {
        ...
        buttons: [
            //csv,
            {
                extend: 'csvHtml5',
                text: 'Exportar CSV',
                filename: 'Comfuavinav - Alistamiento de combustibles y lubricantes (ACL)',
                title: '',
                exportOptions: {
                    columns: [0, 1, 2]
                },
            ...
            //'excel',
            {
                ...
                text: 'Exportar Excel',
                filename: 'Comfuavinav - Alistamiento de combustibles y lubricantes (ACL)',
                title: 'Comfuavinav - Alistamiento de combustibles y lubricantes (ACL)',
                exportOptions: {
                    columns: [0, 1, 2]
                },
...
function eliminar(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                type: "POST",
                url: '/ComfuavinavAlistamientoCombustibleLubricante/Eliminar',
                data: {
                    'Id': id
                },
                beforeSend: function () {
...
function eliminarCarga() {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                type: "POST",
                url: '/ComfuavinavAlistamientoCombustibleLubricante/EliminarCarga',
                data: {
                    'Id': id
                },
                beforeSend: function () {
...
function nuevaComfuavinavAlistamientoCombustibleLubricante() {
    $('#listar').hide();
    $('#nuevo').show();
}
...
function mostrarDatos() {
    ...
    $.ajax({
        type: "POST",
        url: 'ComfuavinavAlistamientoCombustibleLubricante/MostrarDatos',
        data: formData,
        processData: false,
        contentType: false,
        beforeSend: function () {
...
function optReporte(id) {
    ...
    reporteSeleccionado = '/ComfuavinavAlistamientoCombustibleLubricante/ReporteARTR';
}
